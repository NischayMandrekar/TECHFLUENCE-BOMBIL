...
const Page = ({ params }: { params: Promise<{ roomid: string }> }) => {
  ...
  const connectSocket = () => {
    ...
    try {
      ...
      socketRef.current.on("reconnect_attempt", (attemptNumber) => {
        log(
          `Socket.io reconnection attempt ${attemptNumber}/${maxReconnectAttempts}`,
          "info",
          attemptNumber === 1
        );
  ...
  const checkCameraStatus = () => {
    ...
    if (videoElement && videoElement.srcObject instanceof MediaStream) {
      ...
      if (isActive !== cameraActive) {
        log(
          `Camera status changed to: ${isActive ? "active" : "inactive"}`,
          "info",
          true
        );
        ...
        if (socketRef.current && socketRef.current.connected) {
          socketRef.current.emit(isActive ? "camera_on" : "camera_off", {
            userId,
          });
          log(
            `Sent '${isActive ? "camera_on" : "camera_off"}' signal to server`
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (!inMeeting || !cameraActive) {
      log(
        "Not in meeting or camera not active, skipping frame capture setup",
        "debug"
      );
      return;
    }
    ...
    const captureAndSendFrame = async () => {
      ...
      try {
        ...
        if (frameCountRef.current % 100 === 0) {
          log(
            `Frames sent: ${frameCountRef.current}, Captions received: ${captionCountRef.current}`
          );
        }
      } catch (e) {
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="absolute top-2 left-2 z-10 flex items-center space-x-2">
        ...
        <span className="text-xs text-gray-700 bg-white/70 px-2 py-1 rounded">
          {socketStatus === "connected"
            ...
            ? "Connecting..."
            : socketStatus === "error"
            ? `Error (${reconnectAttempts}/${maxReconnectAttempts})`
            : "Disconnected"}
        </span>
      </div>
      ...
      <div className="absolute bottom-4 right-4 z-10">
        ...
        >
          <span className="material-icons">
            {showCaptions ? "closed_caption" : "closed_caption_off"}
          </span>
        </button>
      </div>
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="absolute top-2 right-2 bg-white/80 p-2 rounded text-xs z-10">
          ...
          {lastCaptionTime && (
            <div>
              Last: {Math.round((Date.now() - lastCaptionTime) / 1000)}s ago
            </div>
          )}
        </div>
      )}
